...
# Japan/Asia subset (for panel B occurrence comparison)
occ_unique_2 <- occ_unique[
  occ_unique$lon >=  99.99986 & occ_unique$lon <= 179.9999 &
  occ_unique$lat >=  -0.0001392489 & occ_unique$lat <= 83.99986, ]

# =============================================================================#
# 2. ENVIRONMENTAL LAYERS
# =============================================================================#

studyArea_world_2.5 <- terra::rast("biovars_2.5.tif")
names(studyArea_world_2.5) <- gsub("biovars_2.5_", "bio", names(studyArea_world_2.5))

gdd <- terra::rast("CHELSA_gdgfgd5_1981-2010_V.2.1.tif")
gdd <- terra::resample(gdd, studyArea_world_2.5)
studyArea_world_2.5 <- c(studyArea_world_2.5, gdd)
names(studyArea_world_2.5)[
  names(studyArea_world_2.5) == "CHELSA_gdgfgd5_1981-2010_V.2.1"] <- "gdgfgd5"

studyArea_EURO_2.5 <- terra::crop(
  studyArea_world_2.5,
  terra::ext(-15.00014, 69.99986, 29.99986, 83.99986)
)

studyArea_ASIO_2.5 <- terra::crop(
  studyArea_world_2.5,
  terra::ext(99.99986, 179.9999, -0.0001392489, 83.99986)
)
...
# one occurrence per raster cell (Japan/Asia)
cells2 <- terra::cellFromXY(studyArea_ASIO_2.5[[1]], as.matrix(occ_unique_2[, c("lon", "lat")]))
occ_final_2 <- occ_unique_2[!duplicated(cells2), ]
occ_final_vect_2 <- terra::vect(occ_final_2, geom = c("lon", "lat"), crs = "EPSG:4326")
occs_matrix_2 <- terra::extract(studyArea_ASIO_2.5, occ_final_vect_2)[-1]
has_data_2 <- complete.cases(occs_matrix_2)
occ_final_2 <- occ_final_2[has_data_2, ]
...
env_M_eu  <- terra::mask(terra::crop(studyArea_world_2.5, M_eu), M_eu)
...
env_M  <- terra::mask(terra::crop(studyArea_world_2.5, M_eu), M_eu)
clim_M <- as.data.frame(terra::values(env_M))
clim_M <- na.omit(clim_M)
clim_M <- clim_M[varsenv]

pca_b <- prcomp(clim_M, center = TRUE, scale. = TRUE)

clim_occ_eu <- occs_matrix_1[varsenv]

# --- Japan: keep only octoploid (8x) occurrences ----------------------------#
# studyArea_japan: climate layers cropped/masked to Japan, same variables as
# studyArea_world_2.5 (built analogously to studyArea_EURO_2.5)
studyArea_japan <- terra::crop(
  studyArea_world_2.5,
  terra::ext(99.99986, 179.9999, -0.0001392489, 83.99986)
)

PloidyLevel_R.japo_Japa <- readxl::read_xlsx(
  file.path(reynoutria_folder, "PloidyLevel_R.japo_Japan.xlsx")
)
PloidyLevel_R.japo_Japa <- PloidyLevel_R.japo_Japa %>%
  mutate(lat = gsub("N ", "", Latitude),
        lon = gsub("E ", "", Longitude))

is_dms    <- grepl("\u00b0", PloidyLevel_R.japo_Japa$lat)
ploi_lats <- gsub("N", "", PloidyLevel_R.japo_Japa$Latitude[is_dms])
deg       <- as.numeric(sub("\u00b0.*", "", ploi_lats))
minu      <- as.numeric(sub(".*\u00b0([0-9]+).*", "\\1", ploi_lats))
lat_dec   <- deg + minu / 60

ploi_lons <- gsub("E", "", PloidyLevel_R.japo_Japa$Longitude[is_dms])
deg       <- as.numeric(sub("\u00b0.*", "", ploi_lons))
minu      <- as.numeric(sub(".*\u00b0([0-9]+).*", "\\1", ploi_lons))
lon_dec   <- deg + minu / 60

PloidyLevel_R.japo_Japa$lon <- as.numeric(PloidyLevel_R.japo_Japa$lon)
PloidyLevel_R.japo_Japa$lat <- as.numeric(PloidyLevel_R.japo_Japa$lat)
PloidyLevel_R.japo_Japa[is_dms, c("lon", "lat")] <- cbind(lon = lon_dec, lat = lat_dec)

ploidy <- PloidyLevel_R.japo_Japa[c("lon", "lat", "Ploidy")]
ploidy <- cbind(ploidy, terra::extract(studyArea_japan, ploidy[c("lon", "lat")]))
ploidy <- ploidy[complete.cases(ploidy), ]
ploidy_8x <- ploidy[ploidy$Ploidy == "8x", ]

cells_8x       <- terra::cellFromXY(studyArea_japan[[1]], as.matrix(ploidy_8x[c("lon", "lat")]))
occ_final_3_8x <- ploidy_8x[!duplicated(cells_8x), ]

clim_occ_jap <- occ_final_3_8x[varsenv]
...
# =============================================================================#
# 6. PANEL C - MaxEnt probability of presence (6-class map, Europe)
# =============================================================================#
...
# =============================================================================#
# 7. PANEL D - violin / beeswarm of probability vs bioclim variable
#    with 2.5% / 97.5% quantile lines and max/min text labels
# =============================================================================#
